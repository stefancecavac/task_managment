...
const TaskCard = ({ task }) => {
  ...
  const isTaskFailed = new Date(task.end_date) < new Date();

 
  if (isTaskFailed && task.task_status !== 'failed') {
    handleUpdate('failed');
  }
  ...
  return (
    ...
    <div className={`taskcard ${task.task_status === 'completed' ? 'completed' : task.task_status === 'failed' ? 'failed' : 'in-progress'}`}>
